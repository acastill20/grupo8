...
BEGIN
    ...
    IF 'generation' NOT IN (SELECT column_name FROM information_schema.columns WHERE table_name='pokemon') THEN
        ALTER TABLE pokemon ADD generation int;
        UPDATE pokemon SET generation = 1;
    END IF;
    ...
    IF pid NOT IN (SELECT id from pokemon) THEN
        INSERT INTO pokemon values(pid, pname, ptype, total, hp, attack, defense, sp_atk, sp_def, speed, legendary, 1);
